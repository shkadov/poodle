...
#Dovecot в файле /etc/dovecot/dovecot.conf добавляем ssl_protocols = !SSLv2 !SSLv3 перед строкой ssl_cipher_list блаблабла, рестартуем довкот
#Exim в файле /etc/exim.conf меняем tls_require_ciphers = блаблабла на tls_require_ciphers = ALL:!ADH:+HIGH:+MEDIUM:-LOW:-SSLv2:-SSLv3:-EXP. Рестартуем exim
